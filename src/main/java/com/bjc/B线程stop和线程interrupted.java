...
public class B线程stop和线程interrupted {
     ...
     * 在写入的时候突然stop会造成写入线程的立即死亡而释放锁,则此时对象中数据可能会不一致,而读取线程则可以获得锁而读取到错误的不一致的数据
     * <p>
     * 在写入的时候interrupted 则 只是告诉线程准备中断,而不是立即死亡,从而未立即释放锁,而是等到数据被修改完全后才释放锁,因而数据可以保证读取的时候是一致的.
     * 这个就是stop 和 interrupted的区别
     */
    @Data
